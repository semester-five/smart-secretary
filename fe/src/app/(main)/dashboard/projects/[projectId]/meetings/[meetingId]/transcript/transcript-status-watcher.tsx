...
export function TranscriptStatusWatcher({
  ...
  const { data: status } = useQuery({
    ...
    refetchInterval: (query) => {
      ...
      if (
        data?.latest_job?.status === "queued" ||
        data?.latest_job?.status === "running"
      )
        return 3000;
      return false;
    },
    ...
    <Card className="mb-6">
      <CardHeader>
        <CardTitle className="flex items-center justify-between gap-3">
          ...
          <Badge
            variant={
              status.meeting_status === "failed" ? "destructive" : "secondary"
            }
          >
        ...
        <CardDescription>
          This page will refresh automatically when the transcript is ready.
        </CardDescription>
      </CardHeader>
